...
import {
  ...
  AUTO, CGA_PALETTE, Ellipse, Image, Rectangle, Shape, Text, Triangle, add, assert, body, clamp,
  // eslint-disable-next-line no-unused-vars
...
new Triangle(
	...
	new Ellipse(
		...
		{ anchor: body(w(0), h(1/2)), end: 1/2 }
	),
	new Ellipse(
		w(1/2), w(1/2), body(w(1), h(1)),
		{ anchor: body(w(1), h(1/2)), end: 1/2 }
	)
)
`;
...
class App extends HTMLElement {
  ...
  #loadModel() {
    ...
    if (!name) {
      let model = getModels()[0] ?? createModel({ name: "", text: EXAMPLE });
      location.hash = `#${model.name}`;
      this.#loadModel();
      return;
    }
